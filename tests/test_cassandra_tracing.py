...
class TestCassandra(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        session.execute("INSERT INTO test.luthier (name, location) VALUES ('Mina', 'Eesti')")
        session.execute("INSERT INTO test.luthier (name, location) VALUES ('Tema', 'Eesti Jälle')")
    ...
    def assertTags(self, tags, keyspace, statement, command, paginated=False, exc_msg=False, additional_tags=None):
        ...
        if exc_msg:
            self.assertTrue(tags['error'])
            self.assertEqual(tags['exception'], exc_msg)
    ...
    def test_select_error(self):
        ...
        self.assertEqual(len(spans), 1)
        span = spans[0]
        self.assertTags(span.tags, 'test', statement, 'SELECT', exc_msg='Error from server: code=2200 [Invalid query] message="Undefined column name no"')

    def test_simple_statement_select_custom_tags(self):
        self.session = self.cluster.connect('test')
        self.qt = QueryTracing(self.session, tracer=self.tracer, span_tags={'one': 'more', 'tag': 2})
        statement = 'SELECT * FROM luthier'
        self.session.execute(statement)
        spans = self.tracer.finished_spans()
        self.assertEqual(len(spans), 1)
        span = spans[0]
        self.assertEqual(span.operation_name, statement)
        self.assertTags(span.tags, 'test', statement, 'SELECT', additional_tags={'one': 'more', 'tag': 2})
